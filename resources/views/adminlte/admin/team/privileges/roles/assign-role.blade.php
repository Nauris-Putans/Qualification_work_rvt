...
@section('content')
    <form method="post" action="{{ URL::route('admin.assign-role.store') }}">
        @csrf
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                ...
                <div class="card card-outline card-primary">
                    ...
                    <div class="card-body">
                        {{ Form::component('assignRoleForm', 'components.form.adminlte.admin.assign-role-form', ['users' => $users, 'roles' => $roles]) }}
                        {{ Form::assignRoleForm() }}
                    </div>
                </div>
            </div>

            {{-- Assign Role to Member box--}}
            <div class="col-md-9">
                <div class="card card-outline card-primary">
                    <div class="card-header">
                        <h1 class="card-title">{{ __('Assign Role to Member') }}</h1>
                        <div class="card-tools">
                            <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
                        </div>
                    </div>
                    <div class="card-body">
                        {{-- Data table --}}
                        <table class="table table-striped table-bordered dt-responsive nowrap TableStyle" id="assignRole-table">
                                ...
                                <tr>
                                    <th scope="row">
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" name="role[]" class="role" value={{ $role->id }} >
                                        </div>
                                    </th>
...
@section('js')
    <script>
        // Checks only 1 checkbox at time
        jQuery(document).ready(function(){
            jQuery('[type="checkbox"]').change(function(){
                if(this.checked){
                    jQuery('[type="checkbox"]').not(this).prop('checked', false);
                }
            });
        });
        ...
        function filterColumn ( i ) {
            jQuery('#assignRole-table').DataTable().column( i ).search(jQuery('#col'+i+'_filter').val()).draw();
        }

        // Table
        jQuery(document).ready(function() {
            const table = jQuery('#assignRole-table').DataTable(
                {
                    // Specific columns
                    columnDefs: [
